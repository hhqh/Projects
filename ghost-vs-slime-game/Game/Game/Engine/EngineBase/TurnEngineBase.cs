...
namespace Game.Engine.EngineBase
{
    ...
    public class TurnEngineBase : ITurnEngineInterface
    {
        ...
        {
            if (EngineSettings.BattleMessagesModel.DamageAmount >= Target.CurrentHealth)
            {
                if (EngineSettings.BattleSettingsModel.AllowMiracleMax)
                {
                    if (Target.PlayerType == PlayerTypeEnum.Character)
                    {
                        if (EngineSettings.MiracleMax == 1)
                        {
                            EngineSettings.MiracleMax--;
                            Target.CurrentHealth = Target.MaxHealth;
                            EngineSettings.BattleMessagesModel.MiracleMax = "Miracle Max to the Rescue! Miracle Max Count: " + EngineSettings.MiracleMax + " Current Health: " + Target.CurrentHealth;
                            return false;
                        }
                    }
                }
        ...
        public virtual bool CalculateExperience(PlayerInfoModel Attacker, PlayerInfoModel Target)
        {
            if (Attacker.PlayerType == PlayerTypeEnum.Character)
            {
                ...
                if (LevelUp)
                {
                    EngineSettings.BattleMessagesModel.LevelUpMessage = Attacker.Name + " is now Level " + Attacker.Level + "\nWith Health Max of " + Attacker.GetMaxHealthTotal;
